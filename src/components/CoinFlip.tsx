...
const CoinSpinner = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (loopsRemaining > 0) {
        animationFrameId = requestAnimationFrame(() => setTimeout(animate, 1000 / fps));
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen">
      ...
      <button
        onClick={handleFlip}
        className="mt-6 px-6 py-2 bg-yellow-500 text-white font-semibold rounded shadow hover:bg-yellow-600 transition"
        disabled={spinning}
      >
